...
export function ShareBubbleDialog({
  bubbleId,
  open,
  onOpenChange,
  ...
  const fetchShareStatus = async () => {
    setIsLoading(true);
    try {
      const response = await fetch(`/api/bubbles/${bubbleId}/share`);
      if (response.ok) {
  ...
  const handleToggleSharing = async (enabled: boolean) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setShareStatus(data);
        toast.success(
          enabled ? t("dialog.sharingEnabled") : t("dialog.sharingDisabled")
        );
      } else {
  ...
  const handleRegenerateCode = async () => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        setShareStatus(data);
        toast.success(t("dialog.codeRegenerated"));
      } else {
        const error = await response.json();
        toast.error(error.error || t("dialog.error"));
      }
  ...
  const handleCopyLink = async () => {
    ...
    try {
      ...
      setCopied(true);
      toast.success(t("dialog.linkCopied"));
  ...
  const handleShareWhatsApp = () => {
    if (!shareStatus?.shareUrl) return;
    const text = t("dialog.whatsappMessage");
  ...
  const handleShareEmail = () => {
    if (!shareStatus?.shareUrl) return;
    const subject = t("dialog.emailSubject");
    const body = t("dialog.emailBody", { url: shareStatus.shareUrl });
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                <Label htmlFor="share-toggle">{t("dialog.enableSharing")}</Label>
                <p className="text-sm text-muted-foreground">
                  {t("dialog.enableSharingDescription")}
                </p>
              </div>
              <Switch
                id="share-toggle"
            ...
            {shareStatus?.shareEnabled && shareStatus.shareUrl && (
              <>
                ...
                <div className="space-y-2">
                  <Label>{t("dialog.shareLink")}</Label>
